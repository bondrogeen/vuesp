...
const minValue = (min: number, v: number) => v && +v >= min;
...
export const minVal = (i: number) => (v: number) => minValue(i, v) || `Min value ${i}`;
export const maxVal = (i: number) => (v: number) => maxValue(i, v) || `Max value ${i}`;
