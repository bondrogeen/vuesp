<template>
	<v-dropdown class="v-autocomplete" v-bind="$attrs">
		<template #activator="{ value, on }">
			<v-input :value="value ? search : currentUser" :placeholder="value ? 'Поиск' : placeholder" @click="on.click" @input="onInput" @focus="search = ''">
				<template #icon>
		...
		<div v-if="isEmpty" class="v-autocomplete__empty">Не найдено</div>
	</v-dropdown>
</template>
