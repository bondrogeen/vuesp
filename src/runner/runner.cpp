...
void ScriptRunner::emitEvent(uint32_t hash) {
  ...
  for (int i = 0; i < _eventHandlerCount; i++) {
    if (_eventHandlers[i].active && _eventHandlers[i].hash == hash) {
      if (_logProvider) {
        char buf[64];
        snprintf(buf, 64, "[Runner] Executing handler slot %d", _eventHandlers[i].slotId);
        _logProvider(buf);
      }
...
bool ScriptRunner::removeEventHandler(uint32_t hash) {
  for (int i = 0; i < _eventHandlerCount; i++) {
    if (_eventHandlers[i].active && _eventHandlers[i].hash == hash) {
      ...
      if (_logProvider) {
        char buf[64];
        snprintf(buf, 64, "[Runner] Event handler removed: hash=%u", hash);
        _logProvider(buf);
      }
      return true;
    }
...
void ScriptRunner::initSlotPools() {
  ...
  for (int i = 0; i < POOL_64_COUNT; i++) {
    _slots[idx].slotSize = POOL_64_SIZE;
    _slotPoolType[idx] = 0;
    idx++;
  }
  for (int i = 0; i < POOL_128_COUNT; i++) {
    _slots[idx].slotSize = POOL_128_SIZE;
    _slotPoolType[idx] = 1;
    idx++;
  }
  for (int i = 0; i < POOL_256_COUNT; i++) {
    _slots[idx].slotSize = POOL_256_SIZE;
    _slotPoolType[idx] = 2;
    idx++;
  }
  for (int i = 0; i < POOL_512_COUNT; i++) {
    _slots[idx].slotSize = POOL_512_SIZE;
    _slotPoolType[idx] = 3;
    idx++;
  }
}
...
bool ScriptRunner::handleOn(const Params& params, ScriptState& s, uint32_t now) {
  ...
  if (slot == -1) {
    setError("No suitable slot for event handler");
    return false;
  }
  ...
  if (_logProvider) {
    char buf[128];
    snprintf(buf, 128, "[Runner] Event handler registered: '%s' -> slot %d (id=%d, size=%d, len=%d)",
...
bool ScriptRunner::handleRemoveOn(const Params& params, ScriptState& s) {
  ...
  uint32_t eventHash = ScriptRunner::hash(eventName);
  return removeEventHandler(eventHash);
}

ScriptRunner::ScriptRunner(ScriptConflict defaultStrategy)
    : _defaultStrategy(defaultStrategy), _dataProvider(nullptr), _logProvider(nullptr), _portProvider(nullptr), _stateChangeProvider(nullptr), _lastStateChangeTime(0), _eventHandlerCount(0) {
  _instance = this;
...
int ScriptRunner::findFreeSlot(uint16_t scriptLen) const {
  for (int i = 0; i < TOTAL_SLOTS; i++) {
...
bool ScriptRunner::removeScript(uint8_t id) {
  ...
  if (slot == -1) {
    if (_logProvider) {
      char buf[64];
      snprintf(buf, 64, "[Runner] Script %d not found for removal", id);
      _logProvider(buf);
    }
    return false;
  }
  ...
  for (int i = 0; i < _eventHandlerCount; i++) {
    if (_eventHandlers[i].active && _eventHandlers[i].slotId == slot) {
      ...
      if (_logProvider) {
        char buf[64];
        snprintf(buf, 64, "[Runner] Event handler removed with script %d", id);
        _logProvider(buf);
      }
    }
  }
  ...
  if (_logProvider) {
    char buf[64];
    snprintf(buf, 64, "[Runner] Script %d removed from slot %d", id, slot);
    _logProvider(buf);
  }
  return true;
}
...
bool ScriptRunner::registerScript(uint8_t id, const char* script) {
  if (id >= EVENT_HANDLER_ID_BASE) {
    if (_logProvider) {
      char buf[64];
      snprintf(buf, 64, "[Runner] ERROR: ID %d is reserved for event handlers (>=%d)", id, EVENT_HANDLER_ID_BASE);
      _logProvider(buf);
    }
    return false;
  }
  ...
  if (existing != -1) {
    if (_slots[existing].isHandler) {
      if (_logProvider) {
        char buf[64];
        snprintf(buf, 64, "[Runner] ERROR: ID %d is used by event handler in slot %d!", id, existing);
        _logProvider(buf);
      }
      return false;
    }

    if (_slots[existing].slotSize < len) {
      int newSlot = findFreeSlot(len);
      if (newSlot == -1) {
        if (_logProvider) _logProvider("[Runner] ERROR: No suitable slot for script update!");
        return false;
      }

      ScriptState temp = _slots[existing];
      resetScriptState(existing);

      _slots[newSlot] = temp;
      _slots[newSlot].registered = true;
      strcpy(_slots[newSlot].script, script);
      _slots[newSlot].scriptLen = len;
      _slots[newSlot].pos = 0;
      _slots[newSlot].active = false;

      if (_logProvider) {
        char buf[64];
        snprintf(buf, 64, "[Runner] Script %d moved from slot %d to %d (size: %d -> %d)",
                 id, existing, newSlot, _slots[existing].slotSize, _slots[newSlot].slotSize);
        _logProvider(buf);
      }
      return true;
    }
    ...
    if (_logProvider) {
      char buf[64];
      snprintf(buf, 64, "[Runner] Script %d updated in slot %d (size: %d, len: %d)",
               id, existing, _slots[existing].slotSize, len);
      _logProvider(buf);
    }
    return true;
  }
  ...
  if (slot == -1) {
    if (_logProvider) {
      char buf[64];
      snprintf(buf, 64, "[Runner] ERROR: No suitable slot for script (len=%d)!", len);
      _logProvider(buf);
    }
    return false;
  }
  ...
  if (_logProvider) {
    char buf[64];
    snprintf(buf, 64, "[Runner] Script %d registered in slot %d (size: %d, len: %d)",
             id, slot, _slots[slot].slotSize, len);
    _logProvider(buf);
  }
  return true;
}
...
bool ScriptRunner::runScript(uint8_t id) {
  if (id >= EVENT_HANDLER_ID_BASE) {
    if (_logProvider) {
      char buf[64];
      snprintf(buf, 64, "[Runner] ERROR: Cannot run event handler (id=%d)", id);
      _logProvider(buf);
    }
    return false;
  }
  ...
  if (slot == -1) {
    if (_logProvider) {
      char buf[64];
      snprintf(buf, 64, "[Runner] ERROR: Script %d not found!", id);
      _logProvider(buf);
    }
    return false;
  }
  ...
  if (_slots[slot].isHandler) {
    if (_logProvider) {
      char buf[64];
      snprintf(buf, 64, "[Runner] ERROR: Slot %d is an event handler, not a script!", slot);
      _logProvider(buf);
    }
    return false;
  }
  ...
  if (_slots[slot].active) {
    if (_logProvider) {
      char buf[64];
      snprintf(buf, 64, "[Runner] Script %d already running, RESTART", id);
      _logProvider(buf);
    }
    _slots[slot].active = false;
  }
  ...
  if (_logProvider) {
    char buf[64];
    snprintf(buf, 64, "[Runner] Script %d started", id);
    _logProvider(buf);
  }
  return true;
}
...
bool ScriptRunner::runScriptFrom(uint8_t slot, uint16_t offset, uint16_t len) {
  ...
  if (_slots[slot].active) {
    if (_logProvider) {
      char buf[64];
      snprintf(buf, 64, "[Runner] Script slot %d already running, RESTART", slot);
      _logProvider(buf);
    }
    _slots[slot].active = false;
  }
  ...
  if (_logProvider) {
    char buf[64];
    snprintf(buf, 64, "[Runner] Script slot %d started from offset %d (len=%d)", slot, offset, len);
    _logProvider(buf);
  }
  return true;
}
...
bool ScriptRunner::stopScript(uint8_t id) {
  ...
  if (_logProvider) {
    char buf[64];
    snprintf(buf, 64, "[Runner] Script %d stopped", id);
    _logProvider(buf);
  }
  return true;
}
...
bool ScriptRunner::processCommand(const char* token, ScriptState& s, uint32_t now) {
  ...
  else if (strcmp(cmd, "remove_on") == 0)
    result = handleRemoveOn(params, s);
  ...
  for (int i = 0; i < TOTAL_SLOTS; i++) {
    ...
    if (s.pos >= s.scriptLen) {
      ...
      if (!s.isHandler) {
        if (_logProvider) {
          char buf[64];
          snprintf(buf, 64, "[Runner] Auto-removing script %d (slot %d)", s.id, i);
          _logProvider(buf);
        }
        resetScriptState(i);
      } else {
        ...
        if (_logProvider) {
          char buf[64];
          snprintf(buf, 64, "[Runner] Handler %d completed (kept)", s.id);
          _logProvider(buf);
        }
      }
      continue;
...
void ScriptRunner::printSlotInfo() const {
  ...
  for (int i = 0; i < TOTAL_SLOTS; i++) {
    ...
    if (s.registered) {
      ...
      snprintf(buf, 128, "  Slot %2d: ID=%3d | %s | %s | size=%3d | used=%3d | '%s'",
               i, s.id, type, status, s.slotSize, s.scriptLen, s.script);
    } else {
      snprintf(buf, 128, "  Slot %2d: FREE  | size=%3d", i, s.slotSize);
    }
    _logProvider(buf);
  }

  snprintf(buf, 128, "  TOTAL: slots=%d/%d, memory=%d/%d bytes (%.1f%%)",
