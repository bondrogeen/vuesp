...
bool ScriptRunner::parseVarString(uint8_t idx, int32_t& result) {
    ...
    if (_ctx.stringVars[idx][0] == '\0') { result = 0; return true; }
...
bool ScriptRunner::parseVarData(const char* start, int32_t& result, const char** p, DataKind expectedKind) {
    ...
    if (_dataProvider(_nameBuf, expectedKind, dv, false)) {
        switch (expectedKind) {
            case KIND_INT: result = (int32_t)dv.intVal; break;
            case KIND_UINT: result = (int32_t)dv.uintVal; break;
            case KIND_FLOAT: result = (int32_t)(dv.floatVal * 1000.0f); break;
            case KIND_STRING: result = (int32_t)hash((char*)dv.stringVal.data); break;
...
bool ScriptRunner::parseValue(const char** p, ScriptState& s, int32_t& result, DataKind expectedKind) {
    ...
    if (*pos == '$') {
        ...
        if ((type == 'v' || type == 'i' || type == 'f' || type == 's' || type == 'a' || type == 'p') && isDigit(*(pos + 1))) {
            pos++;
            uint8_t idx = 0;
            while (isDigit(*pos)) { idx = idx * 10 + (*pos - '0'); pos++; }
            *p = pos;
            switch (type) {
    ...
    if (*pos == '\'') {
        ...
        *p = pos;
        if (strlen(_strBuf) == 0) { result = 0; return true; }
    ...
    if (isDigit(*pos) || *pos == '.' || *pos == '-') {
        ...
        if (hasDot) {
            ...
            *p = pos;
            ...
            if (end > pos) { *p = end; result = (int32_t)val; return true; }
            return false;
        }
    }
    return false;
...
bool ScriptRunner::parseCondition(const char* token, ScriptState& s) {
    ...
    while (*p && *p != TOKEN_SEPARATOR) {
        ...
        int32_t rightVal;
        if (!parseValue(&p, s, rightVal, KIND_INT)) { s.ifResult = false; return false; }
        ...
        if (!hasResult) { result = condResult; hasResult = true; }
...
bool ScriptRunner::handleWait(const Params& params, ScriptState& s, uint32_t now) {
    if (params.count < 1) { setError("wait needs 1 param", s.id, s.pos); return false; }
    uint32_t duration = parseTime(params.values[0]);
    if (duration > 0) { s.inWait = true; s.waitUntil = now + duration; }
    return true;
}
...
bool ScriptRunner::handleOn(const Params& params, ScriptState& s, uint32_t now) {
    ...
    while (*p && (p - s.script) < s.scriptLen) {
        if (strncmp(p, "on(", 3) == 0) { depth++; p += 3; continue; }
...
bool ScriptRunner::handleAssignment(const char* token, ScriptState& s) {
    ...
    if (!eq) { setError("expected '='", token, s.id, s.pos); return false; }
    ...
    if ((type == 'v' || type == 'i' || type == 'f' || type == 's' || type == 'a' || type == 'p') && isDigit(*p)) {
        isInternal = true;
        while (isDigit(*p)) { index = index * 10 + (*p - '0'); p++; }
    }
    ...
    if (isInternal) {
        ...
        if (type == 'a' && index < MAX_ARRAY_VARS) {
            ...
            if (*right == '$' && right[1] == 'a') {
                ...
                setError("invalid array src", token, s.id, s.pos);
                return false;
            }
            if (*right == '$' && right[1] == 's') {
                right += 2;
        ...
        if (type == 's' && index < MAX_STRING_VARS) {
            if (*right == '\'') {
                if (!parseString(&right, _strBuf)) { setError("invalid string", s.id, s.pos); return false; }
                ...
                } else if (*p2 == '\'') { parseString(&p2, leftStr); }
                while (*p2 && *p2 != '+') p2++;
                if (*p2 == '+') p2++;

                if (*p2 == '\'') { parseString(&p2, rightStr); }
                else if (*p2 == '$' && p2[1] == 'v') {
                    p2 += 2;
        ...
        if (type == 'v' || type == 'i' || type == 'f') {
            ...
            if (s.hasTempResult) {
                ...
                if (p2[1] == 'f') {
                    ...
                    DataKind expected = (type == 'v') ? KIND_UINT : (type == 'i') ? KIND_INT : KIND_FLOAT;
            ...
            if (*p2 == '+' || *p2 == '-' || *p2 == '*' || *p2 == '/' || *p2 == '%' ||
                *p2 == '&' || *p2 == '|' || *p2 == '^' || *p2 == '~' ||
                ...
                char op1 = *p2;
                char op2 = '\0';
                if ((op1 == '<' && p2[1] == '<') || (op1 == '>' && p2[1] == '>')) { op2 = p2[1]; p2 += 2; }
                else p2++;
                ...
                if (resultIsFloat) {
                    ...
                    switch (op1) {
                        ...
                        case '-': resultFloat = l - r; break;
                        case '*': resultFloat = l * r; break;
                        case '/': if (r == 0) { setError("div by zero", s.id, s.pos); return false; } resultFloat = l / r; break;
                        default: setError("unknown op", s.id, s.pos); return false;
                    }
                    ...
                    switch (op1) {
                        ...
                        case '-': resultInt = leftVal - rightVal; break;
                        case '*': resultInt = leftVal * rightVal; break;
                        case '/': if (rightVal == 0) { setError("div by zero", s.id, s.pos); return false; } resultInt = leftVal / rightVal; break;
                        case '%': if (rightVal == 0) { setError("div by zero", s.id, s.pos); return false; } resultInt = leftVal % rightVal; break;
                        case '&': resultInt = leftVal & rightVal; break;
                        case '|': resultInt = leftVal | rightVal; break;
                        case '^': resultInt = leftVal ^ rightVal; break;
    ...
    if (_dataProvider) {
        ...
        if (*right == '\'') {
            const char* p = right;
            if (!parseString(&p, _strBuf)) { setError("invalid string", s.id, s.pos); return false; }
            DataValue dv;
...
bool ScriptRunner::handleSet(const Params& params, ScriptState& s) {
    if (params.count < 3) { setError("set needs 3 params", s.id, s.pos); return false; }
    const char* name = params.values[0];
    if (name[0] != '$') { setError("set: var expected", s.id, s.pos); return false; }
    char type = name[1];
...
bool ScriptRunner::handleGet(const Params& params, ScriptState& s) {
    if (params.count < 2) { setError("get needs 2 params", s.id, s.pos); return false; }
    const char* name = params.values[0];
    if (name[0] != '$') { setError("get: var expected", s.id, s.pos); return false; }
    char type = name[1];
    uint8_t idx = (uint8_t)(name[2] - '0');
    int pos = atoi(params.values[1]);
    uint8_t val = 0;
    if (type == 'a' && idx < MAX_ARRAY_VARS) {
        ...
        else { setError("array bounds", s.id, s.pos); return false; }
    } else if (type == 's' && idx < MAX_STRING_VARS) {
        uint16_t len = strlen(_ctx.stringVars[idx]);
        if (pos >= 0 && pos < len) val = (uint8_t)_ctx.stringVars[idx][pos];
        else { setError("string bounds", s.id, s.pos); return false; }
    } else {
...
bool ScriptRunner::handleLen(const Params& params, ScriptState& s) {
    if (params.count < 1) { setError("len needs 1 param", s.id, s.pos); return false; }
    const char* name = params.values[0];
    if (name[0] != '$') { setError("len: var expected", s.id, s.pos); return false; }
    char type = name[1];
    uint8_t idx = (uint8_t)(name[2] - '0');
    int len = 0;
    switch (type) {
...
bool ScriptRunner::handleChr(const Params& params, ScriptState& s) {
    if (params.count < 1) { setError("chr needs 1 param", s.id, s.pos); return false; }
...
bool ScriptRunner::handleOrd(const Params& params, ScriptState& s) {
    if (params.count < 1) { setError("ord needs 1 param", s.id, s.pos); return false; }
...
bool ScriptRunner::processCommand(const char* token, ScriptState& s, uint32_t now) {
    ...
    if (!open) { setError("expected '('", token, s.id, s.pos); return false; }
    const char* eq = strchr(token, '=');
    char varName[16] = {0};
    char varType = 0;
    uint8_t varIndex = 0;
    ...
    else if (strcmp(cmd, "log") == 0) result = handleLog(params, s);
        ...
        if (callExternalFunction(cmd, paramCount, _funcParams, resultVal)) {
            ...
            if (hasVar) {
                ...
                switch (resultVal.type) {
                    case VAL_INT:
                        if (varType == 'i') { _ctx.intVars[varIndex] = resultVal.intVal; typeMatch = true; }
                        else if (varType == 'v') { _ctx.uintVars[varIndex] = (uint32_t)resultVal.intVal; typeMatch = true; }
                        else if (varType == 'f') { _ctx.floatVars[varIndex] = (double)resultVal.intVal; typeMatch = true; }
                        break;
                    case VAL_UINT:
                        if (varType == 'v') { _ctx.uintVars[varIndex] = resultVal.uintVal; typeMatch = true; }
                        else if (varType == 'i') { _ctx.intVars[varIndex] = (int32_t)resultVal.uintVal; typeMatch = true; }
                        else if (varType == 'f') { _ctx.floatVars[varIndex] = (double)resultVal.uintVal; typeMatch = true; }
                        break;
                    case VAL_FLOAT:
                        if (varType == 'f') { _ctx.floatVars[varIndex] = (double)resultVal.floatVal; typeMatch = true; }
                        else if (varType == 'v') { _ctx.uintVars[varIndex] = (uint32_t)resultVal.floatVal; typeMatch = true; }
                        else if (varType == 'i') { _ctx.intVars[varIndex] = (int32_t)resultVal.floatVal; typeMatch = true; }
                        break;
                    case VAL_STRING:
                        if (varType == 's') {
                ...
                if (!typeMatch) { setError("type mismatch", s.id, s.pos); return false; }
            }
            return true;
        } else {
